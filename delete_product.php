...
<head>
    ...
    <?php if (isset($_COOKIE["email"]) && isset($_COOKIE["password"])){
            if (USER::is_logged_in($bdd, $_COOKIE["email"], $_COOKIE["password"]) && USER::is_admin($bdd, $_COOKIE["email"])){
                if (isset($_GET["id"]) && $_GET["id"]!= "")
                        PRODUCT::delete_product($bdd, $_GET["id"]);
                else {
                    echo "<p>You do not have the rights to delete product.</p>";
    ...
    ?>
    <div class="center-align"><button onclick="location.href='admin.php';" class="btn waves-effect waves-light pulse" type="submit" name="submit">Back</div></br>
    </body>
