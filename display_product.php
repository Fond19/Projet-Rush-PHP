...
<html>
    ...
    if (USER::is_logged_in($bdd, $_COOKIE["email"], $_COOKIE["password"]) /* && USER::is_admin($bdd, $_COOKIE["email"])*/) {
        if (isset($_GET["id"]) && $_GET["id"] != "") {
            PRODUCT::display_product($bdd, $_GET["id"]);
        } else {
            echo "<p>You do not have the rights to display product info.</p>";
        }
    } else {
        echo "<p>You do not have the rights to display product info.</p>";
    }
...
?>
    <div class="center-align" >
                    <button onclick="location.href='index.php';" class="btn waves-effect waves-light pulse" type="submit" name="submit">Back</button>
    </div>
